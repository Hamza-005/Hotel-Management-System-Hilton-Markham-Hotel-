...
public class AdminServiceShowAll extends JFrame {

    private static final long serialVersionUID = 1L;
    private JTextField serviceIdTextField;

    private final ArrayList<String []> allService = Resources.dataBase.readAllServices();

    public AdminServiceShowAll() {
        super("All Services");
        ...
        printBtn.addActionListener(e ->{
            ...
            try{
                ...
                inp= new FileInputStream(new File("AdminAllServices.jrxml"));
                jd=JRXmlLoader.load(inp);
                jr=JasperCompileManager.compileReport(jd);
                jp=JasperFillManager.fillReport(jr,null,con);
                outp=new FileOutputStream(new File("AdminAllServicesReport.pdf"));
        ...
        addServiceBtn.setFont(new Font("Tahoma", Font.BOLD, 18));
        addServiceBtn.setBorder(new LineBorder(new Color(0, 0, 0), 3));
        addServiceBtn.setBackground(Color.WHITE);
        addServiceBtn.setBounds(901, 25, 205, 67);
        contentPane.add(addServiceBtn);
        addServiceBtn.addActionListener(e -> {
            new AdminServiceAdd().setVisible(true);
            dispose();
        });
        ...
        editBtn.addActionListener(e -> {
            ...
            if (!(id.matches("\\d+"))){
                ...
                InstanceClassServices serviceObj = Resources.dataBase.findServiceByID(Integer.parseInt(id));
                if (serviceObj == null){
                    JOptionPane.showMessageDialog(null, "Service ID not found");
                } else {
                    new AdminServiceEdit(serviceObj).setVisible(true);
                    dispose();
                }
            }
        });
            ...
            if (!(id.matches("\\d+"))){
                ...
                if (serviceObj == null){
                    ...
                    if (result == JOptionPane.YES_OPTION){
                        if (Resources.dataBase.deleteServiceByID(Integer.parseInt(id))){
