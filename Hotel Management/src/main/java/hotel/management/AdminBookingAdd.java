...
public class AdminBookingAdd extends JFrame {
	...
	public AdminBookingAdd(){
		super("Admin Add Booking");
		...
		whiteBackground4.setBounds(52, 425, 193, 41);
		contentPane.add(whiteBackground4);
                
                JLabel GuestIDLabel = new JLabel("Guest ID");
		GuestIDLabel.setFont(new Font("Tahoma", Font.BOLD, 18));
		GuestIDLabel.setBackground(Color.WHITE);
		GuestIDLabel.setBounds(10, 0, 109, 41);
		whiteBackground4.add(GuestIDLabel);
                
                idTextField = new JTextField();
		idTextField.setFont(new Font("Tahoma", Font.BOLD, 18));
		idTextField.setBorder(new LineBorder(new Color(0, 0, 0), 3));
		idTextField.setBounds(288, 421, 263, 45);
		...
		bookBtn.addActionListener(e -> {
			...
			if (selectedType.equals("Select Type")){
				...
				int newBookingId = Resources.dataBase.getLastBookingID() + 1;
                                Integer guestid = Integer.valueOf(idTextField.getText());
				InstanceClassBooking booking = new InstanceClassBooking(newBookingId, guestid, roomSearched.getRoomID(),selectedDateCheckInValue,selectedDateCheckOutValue);

				if (Resources.dataBase.createBooking(booking)){
					JOptionPane.showMessageDialog(null,"Room Booked Successfully");
					Resources.dataBase.updateRoomStatus(roomSearched.getRoomID(),false);
					new AdminBookedRoomShowAll().setVisible(true);
					dispose();
				}
				else{
					JOptionPane.showMessageDialog(null,"Error in booking room");
				}
			}
		});
