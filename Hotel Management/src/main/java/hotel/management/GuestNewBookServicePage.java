...
public class GuestNewBookServicePage extends JFrame {
	...
	public GuestNewBookServicePage() {
		super("Guest New Service Booking");
		...
		try {
			selectedBookingId = Resources.dataBase.fetchBookingsByGuestID()[0];
		} catch (Exception e) {
			selectedBookingId = "-1";
		}
		...
		String[] bookingNumbersArray = Resources.dataBase.fetchBookingsByGuestID();
		...
		bookBtn.addActionListener(e -> {
			...
			if (selectedBookingId.equals("You do not have any booking yet. Please book a room first.")) {
				...
				InstanceClassRoomService roomService = new InstanceClassRoomService(serviceBookedId, Integer.parseInt(selectedBookingId), Resources.dataBase.fetchServiceIDByName(selectedServiceId), Integer.parseInt(quantityTextField.getText()), "not delivered");
