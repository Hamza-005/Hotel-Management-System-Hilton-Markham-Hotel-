...
public class GuestSignup extends JFrame {
	...
	public GuestSignup() {
		...
		signupBtn.setBounds(784, 519, 263, 67);
		...
		signupBtn.addActionListener(e -> {
			...
			if (idTextField.getText().isEmpty() || fullNameTextField.getText().isEmpty() || passwordTextField.getPassword().length == 0 || addressTextArea.getText().isEmpty() || emailTextField1.getText().isEmpty() || emailTextField.getText().isEmpty()) {
				...
				JOptionPane.showMessageDialog(null, "Guest with this ID already exists");
			}
				...
				JOptionPane.showMessageDialog(null, "Guest SignUp Successfully now you can login");
				new GuestLogin().setVisible(true);
				dispose();
			}
		});
